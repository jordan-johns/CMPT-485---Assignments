...
vec2_t mul(const vec2_t a, const vec2_t b);
...
// Component-wise clamping to the range: [a,b]
//  Assumes a <= b
vec2_t clamp(const vec2_t v, const float a, const float b);
vec3_t clamp(const vec3_t v, const float a, const float b);
vec4_t clamp(const vec4_t v, const float a, const float b);

// Reflect: v reflected about n
//   Assumes n is unit length
vec2_t reflect(const vec2_t v, const vec2_t n);
vec3_t reflect(const vec3_t v, const vec3_t n);
vec4_t reflect(const vec4_t v, const vec4_t n);
